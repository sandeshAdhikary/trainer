...
from trainer.utils import flatten_dict, remove_protected_keys
from trainer.queue import TrainingQueue
...
class Study(ABC):
    ...
    def __init__(self, cfg):
        ...
        self.db = None
        if cfg['study'].get('database'):
            self._setup_db(cfg['study'].get('database'))
        self.queue = None
        if cfg['study'].get('queue'):
            self._setup_queue(cfg['study'].get('queue'))

    def train(self, config, pre_train_func=None, post_train_func=None, pre_init_run=None):
        trainer = self._make_trainer(config['overrides'], pre_init_run=pre_init_run)
        
        if pre_train_func is not None:
            pre_train_func(trainer)
        
        trainer.fit()

        if post_train_func is not None:
            post_train_func(trainer)
    ...
    def run_from_queue(self, config):
        queue_name = config['queue_name']
        default_config_dir = config.get('default_config_dir') or os.environ['DEFAULT_CONFIG_DIR']
        default_config_file = config.get('default_config_file') or os.environ['DEFAULT_CONFIG_FILE']
        self.queue.run_from_queue(queue_name, default_config_dir, default_config_file)


    def _setup_queue(self, config):
        self.queue = TrainingQueue(config)
    ...
    def _make_trainer(self, config, pre_init_run=None):
        ...
        self.trainer = trainer_cls(dict(train_config))

        # Set up model
        model = self._make_model(config, trainer=self.trainer)
    ...
    def _make_logger(self, config, pre_init_run=None):
        ...
        logged_trainer_config = remove_protected_keys(dict(flatten_dict(self.trainer.config)))
        logger_config['trainer'] = flatten_dict(logged_trainer_config)
        logged_model_config = remove_protected_keys(dict(flatten_dict(self.trainer.model.config)))
        logger_config['model'] = flatten_dict(logged_model_config)
