...
from stable_baselines3.common.save_util import recursive_getattr, data_to_json, json_to_data
...
class SB3RLTrainer(RLTrainer):
    ...
    def _create_checkpoint_files(self, temporary_dir=True, archive=True, ckpt_state_args=None, **kwargs):
        ...
        storage.make_archive(["sb3_ckpt.zip", "trainer_ckpt.pt"], zipfile_name=f'{ckpt_name}.zip')
        # Delete original files that have been archived
        storage.delete(files=["sb3_ckpt.zip", "trainer_ckpt.pt"])
    ...
    def setup_evaluator(self):
        ...
        self.evaluator = TrainingRLEvaluator(evaluator_config, self, metrics=None)
