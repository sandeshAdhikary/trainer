...
class Logger(ABC):
    def __init__(self, config: Dict, run=None) -> None:
        ...
        self.project = self.project or 'uncategorized'                                  
        ...
        if self.sw_type == 'tensorboard':
            ...
            if run is None:
                ...
                self._sw = wandb.init(project=project, dir=self.dir, config=flatten_dict(config), tags=tags)
                ...
                # run.config = flatten_dict(config)
                for key in flatten_dict(config):
                    if key not in run.config:
                        run.config[key] = flatten_dict(config)[key]
